<div>
    ...
    <div class="mt-1 relative" x-data="{ open: false }">
        <button @class([
            'relative w-full  pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 sm:text-sm',
            'bg-white border  border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500' => !$errors->has(
                $name,
            ),
            'border-red-300 text-red-900 placeholder-red-300 focus:outline-none focus:ring-red-500 focus:border-red-500 ' => $errors->has(
                $name,
            ),
        ]) type="button" @click="open = true" class=""
